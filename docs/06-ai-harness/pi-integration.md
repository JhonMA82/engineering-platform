...
Engineering Platform `0.9.0` se distribuye como paquete Pi nativo. `package.json` declara una extensión, los skills de descubrimiento y evolución, los skills operativos y prompts; Pi los carga globalmente mediante su gestor de paquetes.
...
El instalador conserva una copia versionada en `~/.local/share/engineering-platform/0.9.0`, crea `~/.local/bin/eng` y ejecuta `pi install` sobre esa copia. Si `~/.local/bin` no está en `PATH`, `eng doctor --global` muestra la ruta que falta.
...
```bash
pi install git:github.com/JhonMA82/engineering-platform@v0.9.0
```
